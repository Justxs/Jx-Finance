...
public sealed class ImportPreviewSummary : Summary<ImportPreviewEndpoint, ImportPreviewRequest>
{
    public ImportPreviewSummary()
    {
        ...
        Description = "Parses an exported Swedbank statement and returns the rows it found, each with "
            + "a flag saying whether a matching transaction already exists in the account. Your "
            + "categorization rules are evaluated against each row's description, amount and flow "
            + "type, and the first rule that matches fills in suggestedCategoryId, suggestedTagIds and "
            + "matchedRuleName; a row nothing matched carries none of them. The suggestion is a "
            + "suggestion: confirm sends back whatever the client decided. Nothing is written: this "
            + "call only reads the file. Send the file as multipart/form-data.";
