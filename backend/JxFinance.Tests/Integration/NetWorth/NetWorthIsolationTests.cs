...
public sealed class NetWorthIsolationTests(ApiFixture fixture) : IntegrationTestBase(fixture)
{
    ...
    public async Task Assets_and_debts_of_a_household_partner_stay_out_of_net_worth()
    {
        ...
        var asset = await Seed.AssetAsync(ownerClient, Today, "Flat", currentValue: "90000.00");
        var debt = await Seed.DebtAsync(ownerClient, Today, "Mortgage", outstandingAmount: "60000.00");

        var partnerWorth = await partnerClient.GetFromJsonAsync<NetWorthDto>("/api/networth", TestContext.Current.CancellationToken);
        var updateAsset = await partnerClient.PutAsJsonAsync($"/api/assets/{asset}", new { name = "Mine now", type = "property", currentValue = "1.00", asOf = Today }, TestContext.Current.CancellationToken);
        var updateDebt = await partnerClient.PutAsJsonAsync($"/api/debts/{debt}", new { name = "Mine now", type = "loan", outstandingAmount = "1.00", asOf = Today }, TestContext.Current.CancellationToken);
    ...
    public async Task Deleting_an_asset_or_a_debt_takes_it_out_of_net_worth()
    {
        ...
        var asset = await Seed.AssetAsync(member, Today, "Watch", "valuable", "300.00");
        var debt = await Seed.DebtAsync(member, Today, "Card", outstandingAmount: "120.00");
        ...
        Assert.Equal(HttpStatusCode.NoContent, (await member.DeleteAsync($"/api/assets/{asset}", TestContext.Current.CancellationToken)).StatusCode);
        ...
        Assert.Equal(HttpStatusCode.NoContent, (await member.DeleteAsync($"/api/debts/{debt}", TestContext.Current.CancellationToken)).StatusCode);
