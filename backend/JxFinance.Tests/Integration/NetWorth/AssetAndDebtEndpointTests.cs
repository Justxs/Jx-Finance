...
public sealed class AssetAndDebtEndpointTests(ApiFixture fixture) : IntegrationTestBase(fixture)
{
    ...
    public async Task Revaluing_an_asset_changes_net_worth_by_the_difference()
    {
        ...
        var update = await member.PutAsJsonAsync(
            $"/api/assets/{asset.Id}",
            new { name = "Car, one year on", type = "vehicle", currentValue = "8500.00", asOf = Today });

        update.EnsureSuccessStatusCode();
        var listed = Assert.Single((await member.GetFromJsonAsync<List<AssetDto>>("/api/assets"))!);
        Assert.Equal(new AssetDto(asset.Id, "Car, one year on", "8500.00"), listed);
        Assert.Equal("8500.00", (await member.GetFromJsonAsync<NetWorthDto>("/api/networth"))!.NetWorth);
    }
    ...
    public async Task Paying_down_a_debt_raises_net_worth()
    {
        ...
        var update = await member.PutAsJsonAsync(
            $"/api/debts/{debt.Id}",
            new { name = "Loan", type = "loan", outstandingAmount = "2500.00", interestRate = 5.5m, asOf = Today });

        update.EnsureSuccessStatusCode();
        var listed = Assert.Single((await member.GetFromJsonAsync<List<DebtDto>>("/api/debts"))!);
        Assert.Equal(new DebtDto(debt.Id, "2500.00", 5.5m), listed);
        Assert.Equal("-2500.00", (await member.GetFromJsonAsync<NetWorthDto>("/api/networth"))!.NetWorth);
    }
    ...
    public async Task Create_rejects_an_invalid_value_and_names_the_field(string resource, string field, string value)
    {
        ...
        var response = await Client.PostAsJsonAsync($"/api/{resource}", body);
    ...
    public async Task Updating_or_deleting_an_unknown_record_answers_not_found(string resource)
    {
        var update = await Client.PutAsJsonAsync(
            ...
            new { name = "Ghost", type = resource == "assets" ? "vehicle" : "loan", currentValue = "1.00", outstandingAmount = "1.00", asOf = Today });
        var delete = await Client.DeleteAsync($"/api/{resource}/{Guid.NewGuid()}");
