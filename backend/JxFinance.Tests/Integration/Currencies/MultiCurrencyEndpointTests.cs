...
public sealed class MultiCurrencyEndpointTests(ApiFixture fixture) : IntegrationTestBase(fixture)
{
    ...
    {
        var rate = await Client.GetFromJsonAsync<RateDto>("/api/exchange-rates?from=usd&to=gbp&date=2026-06-03");
    ...
    public async Task Foreign_transaction_is_valued_in_the_reporting_currency_on_its_date()
    {
        ...
        var response = await Client.PostAsJsonAsync(
            "/api/transactions",
            new { accountId = account.Id, type = "income", amount = "110.00", date = "2026-06-02" });
        ...
        var reloaded = await Client.GetFromJsonAsync<AccountDto>($"/api/accounts/{account.Id}");
    ...
    public async Task Transaction_summary_totals_use_the_reporting_currency()
    {
        ...
        await Client.PostAsJsonAsync(
            "/api/transactions",
            new { accountId = account.Id, type = "expense", amount = "55.00", date = "2026-06-04" });
        await Client.PostAsJsonAsync(
            "/api/transactions",
            new { accountId = account.Id, type = "expense", amount = "10.00", currency = "eur", date = "2026-06-04" });

        var summary = await Client.GetFromJsonAsync<SummaryDto>($"/api/transactions/summary?accountId={account.Id}");
    ...
    public async Task Conversion_moves_money_between_currencies_and_books_the_fee()
    {
        ...
        var afterConversion = await Client.GetFromJsonAsync<AccountDto>($"/api/accounts/{account.Id}");
        ...
        var fee = await Client.GetFromJsonAsync<TransactionDto>($"/api/transactions/{conversion.FeeTransactionId}");
        Assert.Equal("2.00", fee!.Amount);

        var listed = await Client.GetFromJsonAsync<PageDto<ConversionDto>>($"/api/conversions?accountId={account.Id}");
        Assert.Equal(conversion.Id, Assert.Single(listed!.Items).Id);

        var deleteResponse = await Client.DeleteAsync($"/api/conversions/{conversion.Id}");
        Assert.Equal(HttpStatusCode.NoContent, deleteResponse.StatusCode);

        var afterDelete = await Client.GetFromJsonAsync<AccountDto>($"/api/accounts/{account.Id}");
        Assert.Equal("1000.00", Assert.Single(afterDelete!.Balances).Amount);
        var feeAfterDelete = await Client.GetAsync($"/api/transactions/{conversion.FeeTransactionId}");
    ...
    public async Task Transfer_between_currencies_records_both_amounts()
    {
        ...
        var missingAmount = await Client.PostAsJsonAsync(
            "/api/transfers",
            new { fromAccountId = euros.Id, toAccountId = dollars.Id, amount = "100.00", date = "2026-06-06" });
        ...
        var from = await Client.GetFromJsonAsync<AccountDto>($"/api/accounts/{euros.Id}");
        var to = await Client.GetFromJsonAsync<AccountDto>($"/api/accounts/{dollars.Id}");
