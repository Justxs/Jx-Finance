...
Failures answer `application/problem+json` shaped by FastEndpoints' RFC 9457 `ProblemDetails`, configured once in `ApiPipelineExtensions`. Validation failures, `AddError` followed by `Send.ErrorsAsync`, and failed service results all land in the same envelope:
