# API surface

Generated from endpoint route declarations on 2026-09-05. The running OpenAPI document is the authoritative request/response schema; regenerate the frontend client after contract changes.

All routes require authentication except first-run setup/status and login, plus public diagnostics where explicitly configured. User administration requires the Admin role. Household permissions are checked in its service.

Money values are strings. Date-only values use YYYY-MM-DD. Collection paging uses page/pageSize and items/page/pageSize/total.

## How endpoints are written

A feature folder is laid out by role, and namespaces mirror the folders:

```
Endpoints/Accounts/
  AccountsGroup.cs              route prefix, OpenAPI tag, shared error responses
  Interfaces/                   IAccountService
  Services/                     AccountService
  Mappers/                      AccountMapper
  Shared/                       AccountResponse, Iban — anything two operations need
  CreateAccount/                endpoint, request, validator, summary
  GetAccount/  GetAccounts/  UpdateAccount/  DeleteAccount/
```

Everything specific to one operation stays in that operation's folder; the moment a second operation needs it, it belongs in `Shared/`. Features without a mapper (Dashboard, Reports, Imports, Ping, Users) simply have no `Mappers/` folder.

Every endpoint is a FastEndpoints class under `Endpoints/<Feature>/<Operation>/`, alongside its request DTO, its FluentValidation validator, and its documentation class. Endpoints hold no data access; they call a feature service and translate its result.

`Configure()` carries the contract:

- The route is declared relative to the feature's `Group`, and the group supplies the `api` prefix, the OpenAPI tag, and the error responses every endpoint in it can return. Groups derive from `Common/ApiGroup.cs`; the two anonymous ones (`Setup`, `Diagnostics`) pass `requiresAuthentication: false` so they do not advertise a 401 they never return.
- `Description()` declares status codes the framework cannot infer: the 201 returned by the create endpoints, the 404 an owner-scoped lookup can return, the 429 from a throttled endpoint, and the file content types of the two export endpoints.
- Cross-cutting settings sit next to the route: `AllowAnonymous()`, `Roles(AppRoles.Admin)`, `Throttle(...)`, `AllowFileUploads()`.

The prose documentation lives next door in `<Operation>Summary.cs`, a `Summary<TEndpoint, TRequest>` (or `Summary<TEndpoint>` where the endpoint takes no request) that FastEndpoints binds to the endpoint by type. It holds a one-line summary, a paragraph of behaviour worth knowing before calling, a description per route and query parameter, a description per status code, and an example request body. This is the only place endpoint documentation lives — Scalar and the generated client both read it out of the OpenAPI document.

Endpoints, validators, mappers, and summaries are all discovered by the FastEndpoints source generator (`FastEndpoints.Generator`) rather than by assembly scanning, so registration and model binding are resolved at compile time. `AddFastEndpoints(JxFinance.Api.DiscoveredTypes.All)` in `ApiServiceExtensions` is what selects that path.

## Mapping

Translation between request DTOs, domain entities, and response DTOs lives in one `Mapper<TRequest, TResponse, TEntity>` per feature, under the feature's `Mappers/` folder — `AccountMapper`, `BudgetMapper`, `CategoryMapper`, `GoalMapper`, `HouseholdMapper`, `AssetMapper`, `DebtMapper`, `NotificationMapper`, `RecurringBillMapper`, `TransactionMapper`, `TransferMapper`. Services inject the mapper and keep only the querying, validation, and persistence.

`ToEntity` takes the create request; `UpdateEntity` is overloaded for the update request, since one mapper serves both. Several responses carry values no entity holds — an account balance, a budget's spend to date, a transaction's split lines, a household's members — so those mappers add a `FromEntity` overload taking the computed value as a second argument, and the service supplies it. Where that overload exists the one-argument `FromEntity` is deliberately left unimplemented: a response built from the entity alone would be wrong.

Endpoints do not carry a mapper generic argument (`Endpoint<TRequest, TResponse, TMapper>`), because the data the responses need comes from queries the service owns rather than from the request.

## OpenAPI document

`Extensions/OpenApiExtensions.cs` holds the document settings: title, version, tag descriptions, the cookie security scheme, and the document-level description.

It also runs `SchemaVariants.Collapse` as a document transformer. Property-level descriptions and examples make ASP.NET Core's OpenAPI generator clone any `$ref`-ed schema that carries them, so a documented `AccountType` property emits `AccountType__op6EE6F961C867` rather than reusing `AccountType` — and the generated TypeScript client then carries those hashed names. The transformer points such properties back at the canonical schema and deletes the clones. A description that every clone of a schema agreed on (`"Weekly, Monthly, Quarterly, or Yearly."`) is promoted onto the canonical schema, so it survives; where endpoints described the same type differently the canonical schema keeps no description and the endpoint's own prose carries it.

## Errors

Failures answer `application/problem+json` shaped by FastEndpoints' RFC 9457 `ProblemDetails`, configured once in `ApiPipelineExtensions`. Validation failures, `AddError`/`ThrowError` calls, and service results all land in the same envelope:

```json
{
  "type": "https://tools.ietf.org/html/rfc9110#section-15.5",
  "title": "One or more validation errors occurred.",
  "status": 400,
  "instance": "/api/accounts",
  "traceId": "0HMPNHL0JHL76:00000001",
  "errors": [{ "name": "name", "reason": "'Name' must not be empty.", "code": "validation" }]
}
```

`name` is the offending request property, or `GeneralErrors` for a failure that is not tied to one field. `code` is the machine-readable reason, and is what clients should branch on rather than the prose in `reason`.

Services return `Result<T>` with one of the codes in `Common/Errors/ErrorCodes.cs`. Endpoints unwrap it with `ValueOrThrow()` (or `EnsureSuccess()` when there is no value), which raises the failure through the FastEndpoints validation context and maps the code to a status: `not_found` to 404, `conflict` to 409, `forbidden` to 403, `unauthorized` to 401, anything else to 400. Nothing reaching for a resource it cannot see is told the difference between "missing" and "not yours": those cases return 404 on purpose.

## Routes

| Method | Route |
| --- | --- |
| GET | `/api/accounts` |
| POST | `/api/accounts` |
| DELETE | `/api/accounts/{id}` |
| GET | `/api/accounts/{id}` |
| PUT | `/api/accounts/{id}` |
| GET | `/api/assets` |
| POST | `/api/assets` |
| DELETE | `/api/assets/{id}` |
| PUT | `/api/assets/{id}` |
| POST | `/api/auth/2fa/disable` |
| POST | `/api/auth/2fa/enable` |
| POST | `/api/auth/2fa/setup` |
| POST | `/api/auth/login` |
| POST | `/api/auth/logout` |
| GET | `/api/auth/me` |
| GET | `/api/budgets` |
| POST | `/api/budgets` |
| DELETE | `/api/budgets/{id}` |
| PUT | `/api/budgets/{id}` |
| GET | `/api/categories` |
| POST | `/api/categories` |
| DELETE | `/api/categories/{id}` |
| PUT | `/api/categories/{id}` |
| GET | `/api/dashboard/category-breakdown` |
| GET | `/api/dashboard/monthly-trend` |
| GET | `/api/dashboard/summary` |
| GET | `/api/debts` |
| POST | `/api/debts` |
| DELETE | `/api/debts/{id}` |
| PUT | `/api/debts/{id}` |
| GET | `/api/goals` |
| POST | `/api/goals` |
| DELETE | `/api/goals/{id}` |
| PUT | `/api/goals/{id}` |
| GET | `/api/households` |
| POST | `/api/households` |
| DELETE | `/api/households/{id}` |
| GET | `/api/households/{id}` |
| PUT | `/api/households/{id}` |
| POST | `/api/households/{id}/members` |
| DELETE | `/api/households/{id}/members/{userId}` |
| PUT | `/api/households/{id}/members/{userId}` |
| POST | `/api/import/swedbank/confirm` |
| POST | `/api/import/swedbank/preview` |
| GET | `/api/networth` |
| GET | `/api/networth/history` |
| GET | `/api/notifications` |
| POST | `/api/notifications/read-all` |
| PATCH | `/api/notifications/{id}/read` |
| GET | `/api/ping` |
| GET | `/api/recurring-bills` |
| POST | `/api/recurring-bills` |
| DELETE | `/api/recurring-bills/{id}` |
| GET | `/api/recurring-bills/{id}` |
| PUT | `/api/recurring-bills/{id}` |
| POST | `/api/recurring-bills/{id}/confirm` |
| GET | `/api/reports/summary` |
| POST | `/api/setup` |
| GET | `/api/setup/status` |
| GET | `/api/transactions` |
| POST | `/api/transactions` |
| GET | `/api/transactions/export` |
| GET | `/api/transactions/export/pdf` |
| DELETE | `/api/transactions/{id}` |
| GET | `/api/transactions/{id}` |
| PUT | `/api/transactions/{id}` |
| GET | `/api/transfers` |
| POST | `/api/transfers` |
| DELETE | `/api/transfers/{id}` |
| GET | `/api/users` |
| POST | `/api/users` |
| PUT | `/api/users/me` |
| POST | `/api/users/{id}/deactivate` |
| PUT | `/api/users/{id}/role` |

Import confirm rows optionally select TransferAccountId and ExistingTransferId. Bill confirmation requires ExpectedDueDate. Transfer listing accepts an optional Date filter. 2FA setup/disable requires Password.
