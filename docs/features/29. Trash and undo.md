...
| Attachment | soft delete; the file stays on disk | yes | a restore is refused while the transaction is deleted, when the transaction already has ten files again, or when the file is gone; 30 days after the delete `AttachmentPurgeJob` removes the row and the file (see [Attachments](<31. Attachments.md>)) |
...
| Notification, session, outbox message | never deleted by a user, or deleted on purpose | n/a | there is nothing a person asked to undo; an expired or revoked session is swept by `RetentionJob` (see [Background jobs](<23. Background jobs.md>)) |
...
Ten of the fourteen kinds are swept: transaction, transfer, conversion, budget, goal, asset, debt, recurring entry, investment entry and categorization rule. A transaction takes its lines, its tag links, its attachment rows and their files with it, and a transfer takes its `TransferImports`; the order the job works in, and the reason a conversion's fee transaction survives while the conversion is alive, are in [Background jobs](<23. Background jobs.md>). Attachments stay with `AttachmentPurgeJob`, which owns the files on disk. Categories, tags and households are not swept at all: restricting foreign keys point at them from rows the sweep does not touch, so hard-deleting one would need a second pass of rewrites to save about a hundred bytes.
