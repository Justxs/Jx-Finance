...
- a delete renumbers what is left in the same way, inside the delete's own database transaction. The deleted row keeps its own `Position`, and the tags it set are recorded beside its trash entry, so Undo or Restore puts it back at that position — clamped to the end if the list has shrunk — renumbers the others and links its tags again. A restore that would make a 101st rule is refused with `collection.invalidSize`. See [Trash and undo](<29. Trash and undo.md#deletes-that-rewrite-other-rows>).
...
There are two places this runs, and they run it differently on purpose.

Over the **ledger** the condition is a `WHERE` clause. `CategorizationRuleService.MatchLedgerAsync` builds one `IQueryable<Transaction>` per rule with the account, amount, flow-type and `EF.Functions.ILike` conditions on it, and selects only the ids that match. Nothing but the ids of the rows a rule claims comes back, so the cost follows the size of the answer and not the size of the ledger. First-match-wins is carried between rules by a growing list of claimed ids, which becomes a `NOT (id = ANY(...))` on the next rule's query.

Over an **import preview** the rows are not in the database yet — they come from a CSV file that the preview call is parsing and will not write. There is nothing to run SQL against, so `RuleMatcher.Matches` answers the same question in memory, with an ordinary case-insensitive comparison. `RuleMatcher` holds both halves side by side, `LikePatternFor` for the SQL path and `Matches` for the in-memory one, so the two definitions of "contains, ignoring case" sit in one file and are read together.
...
flowchart TD
    ...
    Tx --> Update["ExecuteUpdate sets the category of the claimed ids"]
    Update --> Tag["missing TransactionTag rows are inserted;<br/>tags already on the row are kept"]
    Tag --> Answer["the same per-rule counts, now as what changed"]
```

The preview and the run take the same body and answer the same shape, and the preview's numbers are the run's numbers: both call one `MatchLedgerAsync`, so nothing can drift between the count somebody read and the change they approved. The apply step is separate and explicit; the preview writes nothing.
