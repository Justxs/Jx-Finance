...
3. Reuse the shared service helpers in `Common/` instead of repeating their bodies: `FindOrNotFoundAsync`, `UpdateOrNotFoundAsync` and `DeleteOrNotFoundAsync` for the lookup that answers 404, `IReferenceGuard` for "does this account exist" and "is this category of the right type", and `IPagedRequest` with `ToPageAsync` for a paged list.
