...
> [!note] **Single-user for the MVP ([[9. Open questions & decisions|D50]]); sharing landed in M6 for a subset of entities ([[9. Open questions & decisions|D61]]).** Every row still has a `UserId` owner and the base query filter is unchanged. `Account` and `Category` additionally carry `Scope` (`Personal`/`Shared`) + `HouseholdId` — a shared row is visible to every member of that household, not just its owner. `Transaction`, `TransactionLine`, and `Transfer` have no `Scope` of their own; they inherit it from their `Account` through the query filter (a join), so re-scoping an account re-scopes its whole history in one update. Budgets, Goals, Net worth, Recurring bills, and Notifications remain `UserId`-only for now — widening them is the same small additive shape.
...
## Ownership
The base rule that scopes data ([[9. Open questions & decisions|D50]]), plus the sharing rule added on top for a subset of entities in M6 ([[9. Open questions & decisions|D61]]).
...
**Visibility (who can read a row), for entities *without* sharing (Budget, Goal, Asset, Debt, RecurringBill, Notification):**
> `UserId = currentUser`

**Visibility for `Account` and `Category` (`IShareable` — carry their own `Scope` + `HouseholdId`):**
> `UserId = currentUser` OR (`Scope = Shared` AND `HouseholdId ∈` the current user's household memberships)

**Visibility for `Transaction` and `Transfer`** — no `Scope` of their own; inherited from their `Account`:
> `UserId = currentUser` OR (their `Account` is `Shared` AND its `HouseholdId ∈` the current user's memberships)

One central filter adds the relevant rule to *every* query — it's the isolation boundary, not a UI convenience. `Household` and `HouseholdMembership` get their own filter too: visible to any member of that household, not just whoever created the row.

> [!info] **Sharing shipped in M6 for Accounts + Categories only ([[9. Open questions & decisions|D61]]).** Budgets, Goals, Net worth (Asset/Debt), Recurring bills, and Notifications stay `UserId`-only for now — widening them later is the same small additive-migration shape. See [[9. Open questions & decisions#Households & sharing — resolved for M6 (D50 → D61)]] for how the four parked D50 questions were resolved.
...
The **app role** is `Admin` vs `Member` — only `Admin` manages *users*. A second, independent *household* role axis (`Owner`/`Member`, per household — see below) arrived with sharing in M6 ([[9. Open questions & decisions|D61]]); it governs household/member management, not app-wide permissions. See the [[6. API surface#Authorization matrix|authorization matrix]].
...
| Id | uuid | Primary key |
| Name | string | |
| CreatedAt / UpdatedAt / IsDeleted | | Standard `EntityBase` fields (soft-delete) |
...
| UserId | uuid | FK → `AppUser` — the member |
| Role | enum | `Owner` or `Member`, scoped to *this* household. Only an Owner can rename/delete the household or manage members. A household always keeps at least one Owner (enforced on role-change and on remove) |
| CreatedAt / UpdatedAt / IsDeleted | | Standard `EntityBase` fields |

Unique on `(HouseholdId, UserId)` — one membership row per user per household. Visible to any member of that household (not just its own creator) — see [[#Ownership]].
...
| Scope | enum | `Personal` (default) or `Shared` — [[9. Open questions & decisions\|D61]] |
| HouseholdId | uuid? | FK → `Household`, set when `Scope = Shared` |
| CreatedAt | timestamp | |
