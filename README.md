...
`check-fast` needs no Docker: backend format and build, frontend types, lint, format, unit and DOM tests. `check` is what CI runs apart from the story and end-to-end tests: it adds the backend tests, API contract drift and the app and Storybook builds. `test-stories` runs every Storybook story in headless Chromium and `e2e` runs Playwright smoke tests against a throwaway Docker stack on port 8089 that never touches the dev database; both need `just e2e-install` once. [Adding a feature end to end](docs/13.%20Adding%20a%20feature.md) shows where each check fits.
